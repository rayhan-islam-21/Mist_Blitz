...
const AdminLayout = ({ children }) => {
  ...
  const { user, loading } = useContext(AuthContext); // Get both user and loading

  const toggleSidebar = () => setSidebarOpen(!sidebarOpen);

  // --- 1. HANDLE AUTH LOADING ---
  // This blocks the Sidebar from showing the "wrong" role for a second
  // if (loading || !user) {
  //   return (
  ...
  // --- 2. RENDER ACTUAL LAYOUT ---
  return (
    <div className="min-h-screen bg-slate-50 flex overflow-x-hidden">
      
      {/* Sidebar now has guaranteed user data upon mounting */}
      ...
      <div className="flex-1 flex flex-col min-w-0 max-w-full transition-all duration-300 md:ml-64">
        ...
        <header className="md:hidden sticky top-0 z-30 flex items-center justify-between px-4 h-16 bg-white border-b border-gray-200 w-full">
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 bg-red-600 rounded flex items-center justify-center text-white font-bold text-sm">M</div>
