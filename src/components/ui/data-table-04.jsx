    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-2 text-[11px] font-bold text-slate-700 uppercase italic">
          ...
          {row.getValue("memberName") || "FREE_STOCK"}
        </div>
      ),
    },
    {
  ...
  return (
    <div className="min-h-screen bg-white selection:bg-red-600 selection:text-white text-slate-900 font-sans p-4 md:p-10">
      ...
      <Dialog open={editModalOpen} onOpenChange={setEditModalOpen}>
        <DialogContent className="sm:max-w-xl rounded-2xl border-none p-0 overflow-hidden bg-white shadow-2xl">
          <div className="bg-slate-900 px-6 py-4 flex justify-between items-center text-white">
            <div className="flex items-center gap-2">
              <Activity size={16} className="text-red-500" />
              <span className="text-[10px] font-black uppercase tracking-widest">Update_Resource</span>
            </div>
            <button onClick={() => setEditModalOpen(false)} className="hover:rotate-90 transition-transform"><X size={18} /></button>
          </div>
          <div className="p-8 space-y-8">
            <h2 className="text-4xl font-black uppercase italic tracking-tighter text-slate-900 leading-none">Modify <span className="text-red-600">Asset</span></h2>
            <div className="grid grid-cols-2 gap-6">
              <div className="col-span-2 space-y-2">
                <Label className="text-[10px] font-black uppercase text-slate-400">Asset Designation</Label>
                <Input value={editForm.name} readOnly className="rounded-xl border-slate-200 bg-slate-50 h-12 font-bold uppercase italic text-slate-400 cursor-not-allowed" />
              </div>
              <div className="space-y-2">
                <Label className="text-[10px] font-black uppercase text-slate-900">Inventory Count</Label>
                <Input type="number" value={editForm.quantity} onChange={(e) => setEditForm({...editForm, quantity: Number(e.target.value)})} className="rounded-xl border-slate-200 h-12 font-black text-xl focus:ring-red-500 focus:border-red-500" />
              </div>
              <div className="space-y-2">
                <Label className="text-[10px] font-black uppercase text-slate-900">Current Category</Label>
                <Input value={editForm.category} readOnly className="rounded-xl border-slate-200 bg-slate-50 h-12 font-bold uppercase text-slate-400" />
              </div>
            </div>
          </div>
          <div className="p-6 bg-slate-50 flex justify-end gap-4 border-t border-slate-100">
            <button onClick={() => setEditModalOpen(false)} className="px-6 py-2 text-[10px] font-black uppercase text-slate-400 hover:text-slate-900">Cancel</button>
            <button onClick={handleSaveChanges} className="px-10 py-3 bg-red-600 text-white rounded-xl text-[10px] font-black uppercase italic tracking-widest hover:bg-red-700 transition-colors shadow-lg shadow-red-200">Commit Changes</button>
          </div>
        </DialogContent>
      </Dialog>
    </div>
