...
const CheckoutEquipmentTable = () => {
  ...
  const handleOpenCheckout = (asset) => {
    ...
    // Auto-fill from AuthContext (MongoDB structure)
    if (user?.info) {
  ...
  const confirmCheckout = async () => {
    if (!selectedAsset || authLoading) return;

    if (Number(checkoutQty) > selectedAsset.quantity) {
      toast.error("INSUFFICIENT STOCK");
      return;
    }
    
    if (!receiverName || !receiverBlitzId) {
      toast.error("RECEIVER IDENTITY REQUIRED");
      return;
    }
    ...
    try {
      ...
      setEquipment(prev => prev.map(eq => 
        eq._id === selectedAsset._id 
          ? { ...eq, quantity: eq.quantity - Number(checkoutQty) } 
          : eq
      ));
      ...
      toast.error(err.response?.data?.message || "CHECKOUT FAILED", { id: loadId });
    }
  };
  ...
  const columns = useMemo(() => [
        ...
        <div className="flex items-center gap-4 group">
          <div className="relative h-14 w-14 shrink-0 bg-slate-100 rounded-lg overflow-hidden border border-slate-200 group-hover:rotate-2 transition-transform">
            {row.original.image ? (
              ...
              <Package className="m-auto text-slate-300 h-full" size={20} />
            )}
          </div>
          <div>
    ...
    { 
      ...
      cell: ({ row }) => { 
        ...
        return ( 
          <span className={`px-2 py-1 rounded text-[14px] font-mono font-black ${qty > 0 ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'}`}>
          ...
          <table className="w-full border-collapse">
            ...
            <tbody className="divide-y divide-slate-100">
              {dataLoading ? (
                ...
                <tr>
                  <td colSpan={columns.length} className="p-20 text-center font-black text-slate-300 uppercase italic tracking-widest text-sm">
      ...
      <Dialog open={checkoutModalOpen} onOpenChange={setCheckoutModalOpen}>
        <DialogContent className="sm:max-w-xl rounded-none border-[6px] border-black/20 p-0 overflow-hidden bg-white shadow-2xl">
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input value={receiverName} readOnly className="rounded-none font-black text-slate-900 border-2 border-slate-950 h-12 italic uppercase bg-slate-50" />
                </div>
                <div className="space-y-2">
                  <Label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Blitz ID</Label>
                  <Input value={receiverBlitzId} readOnly className="rounded-none text-2xl border-2 border-slate-950 h-12 font-black uppercase bg-slate-50 text-red-600" />
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="bg-slate-50 p-4 border-l-4 border-slate-950 flex justify-between items-center">
                  <span className="text-lg font-black text-slate-950 uppercase">{selectedAsset?.name}</span>
                  <span className="text-[10px] font-mono font-bold text-slate-400">{selectedAsset?.bash}</span>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <div className="space-y-3">
                  ...
                  <Input 
                    ...
                    className="rounded-none border-2 text-black border-slate-950 h-14 font-black text-3xl focus:ring-0" 
                  />
                </div>
              </div>
            </div>
          ...
          <div className="bg-slate-950 p-6 flex items-center justify-between gap-4">
            <button onClick={() => setCheckoutModalOpen(false)} className="px-8 py-4 text-[11px] font-black uppercase tracking-widest text-slate-500 hover:text-white transition-colors">
              Cancel
            </button>
            <button 
              onClick={confirmCheckout} 
              disabled={authLoading || checkoutQty < 1} 
              className="bg-red-600 hover:bg-red-700 text-white px-10 py-4 font-black uppercase italic tracking-widest transition-all border-b-4 border-red-800 active:border-b-0 active:translate-y-1 flex items-center gap-2 disabled:bg-slate-800"
            >
