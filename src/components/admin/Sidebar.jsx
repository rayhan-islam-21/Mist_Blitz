...
const Sidebar = ({ sidebarOpen, toggleSidebar }) => {
  ...
  const handleConfirmLogout = async () => {
    try {
      await logOut(); // Assuming logOut is an async function from your context
      setLogoutModalOpen(false);

      // Push the user to the home page
      router.push("/");

      // Optional: Refresh to clear any cached auth states
      router.refresh();
    } catch (error) {
                      ...
                      <div>
                        <button
                          ...
                          className={`flex items-center w-full px-4 py-3 rounded-xl transition-all group ${
                            hasActiveChild
                              ? "text-red-500 font-semibold bg-red-50/30"
                              : "hover:bg-gray-50"
                          }`}
                        >
                          <span
