...
import { FaUserPlus, FaChevronRight, FaEnvelope, FaLock, FaUserShield } from "react-icons/fa";
...
const AdminRegister = () => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      await handlePostRegistration(result.user, data.name);
          ...
          <div className="p-10 md:p-12">
            ...
            <div className="mb-10 text-center">
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-slate-800/50 border border-slate-700 mb-6">
                ...
                <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Admin access request</span>
              </div>
              ...
              <p className="text-slate-500 text-xs mt-3 tracking-wide">Create secure administrator credentials</p>
            </div>
            ...
            >
              <FcGoogle size={18} />
              <span className="text-xs font-bold text-slate-300 uppercase tracking-widest">Connect with Google</span>
            </button>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
              <div>
                ...
                <div className="relative group">
                  ...
                  <input {...register("name")} type="text" placeholder="e.g. Rayhan Islam" className={inputClasses} />
                </div>
                {errors.name && <p className="text-red-500 text-[10px] mt-1.5 font-medium ml-1">{errors.name.message}</p>}
              </div>
              ...
              <div>
                ...
                {errors.email && <p className="text-red-500 text-[10px] mt-1.5 font-medium ml-1">{errors.email.message}</p>}
              </div>
              ...
              <div>
                ...
                {errors.password && <p className="text-red-500 text-[10px] mt-1.5 font-medium ml-1">{errors.password.message}</p>}
              </div>
              ...
              >
                {isSubmitting ? "Syncing Node..." : (
                  <>
                    Initialize Account <FaChevronRight className="text-[10px] group-hover:translate-x-0.5 transition-transform" />
                  </>
                )}
              </button>
            </form>
            ...
            <div className="mt-10 text-center">
              <p className="text-[10px] font-bold text-slate-600 uppercase tracking-widest">
                Existing Node? 
                <Link href="/auth/login" className="ml-2 text-slate-300 hover:text-red-500 transition-colors underline underline-offset-4 decoration-slate-700 hover:decoration-red-500">
                  Authenticate Here
                </Link>
              </p>
            </div>
